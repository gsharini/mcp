...
class TestGetClientFactories:
    ...
    @pytest.mark.xfail(reason="Superseded by multi-tenant _make_client coverage")
...
def test_server_helpers_cover_serialization_config_and_wrapping(monkeypatch, tmp_path):
    ...
    assert server._build_signer_for_session(
        {"key_file": "key.pem", "security_token_file": str(token_file)}
    ) is signer
    monkeypatch.setenv("BOOLEAN_TEST", "yes")
    assert server._env_bool("BOOLEAN_TEST") is True
    monkeypatch.setenv("ORACLE_MCP_REGION", "us-phoenix-1")
    assert server._effective_region("us-ashburn-1") == "PROFILE2"


@pytest.mark.xfail(reason="Superseded HTTP configuration helpers were removed by the OAuth port")
...
@pytest.mark.xfail(reason="Superseded local-profile helpers were removed by the OAuth port")
...
@pytest.mark.xfail(reason="Prompt return shapes changed in the reference server")
...
class TestServer:
    @pytest.mark.xfail(reason="Superseded HTTP signer was removed by the OAuth port")
    ...
    @patch("os.getenv")
    @pytest.mark.xfail(reason="OAuth transport startup replaces the legacy HTTP path")
